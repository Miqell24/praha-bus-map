...
build.mjs cannot tell the difference. Praha: the road grid covers the city
and its 35 km ring (72 x 68 km), the rail file reaches the ends of the Esko
S-lines (Beroun, Kralupy, Cesky Brod, Benesov), out of the Geofabrik Czech
Republic extract in one pass.
...
PBFS = [os.path.join(ROOT, 'data', 'czech-republic-latest.osm.pbf')]

# must match pipeline/download.sh and build.mjs
S, N, W, E = 49.75, 50.40, 13.95, 14.90
RAIL_BOX = (49.60, 13.60, 50.60, 15.30)
...
rail_file = os.path.join(ROOT, 'data/osm/praha-rail.json')
...
GEN = 'pbf-tiles.py (Geofabrik czech-republic)'
