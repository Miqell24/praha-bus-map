...
// Praha: ONE feed for the whole PID (Prague and Central Bohemia) cut down to
// the city and its ring by a precomputed allowlist (scope.mjs →
// data/scope.json). Four cfgs: buses and trolleybuses (navy, green), trams
// (family red), the metro A/B/C in its own colours, the Esko S-lines in
// PID's navy with the trunk treatment. See MODES.
// Usage: node pipeline/build.mjs [--all | lines...] [--tram all|22,A,S1]
...
// ONE feed for the whole PID, FOUR cfgs.
//
// ROPID publishes a single GTFS for Pražská integrovaná doprava — Prague AND
// the Central Bohemian region, 890 routes from Kladno to Kolín, refreshed
// weekly with a two-week horizon (pid.cz/opendata, data.pid.cz/PID_GTFS.zip).
// The map is Prague and its ring, so pipeline/scope.mjs precomputes the
// allowlist (data/scope.json): 20 km around Můstek for buses and
// trolleybuses, all trams and the metro, the Esko S-lines by a 25 km rule.
// Route types: 3 bus, 11 trolleybus (51–59 and 275, green by the family's
// rule), 0 tram, 1 metro (A green, B yellow, C red — the feed's own colours),
// 2 train (S-lines navy #251E62 as PID prints them), 4 ferry (out — no
// water graph).
//
// LINE KEYS: nothing to invent. PID numbers every line uniquely across the
// system — trams 1–26 and the night 91–99, buses 100–999, night 901–918 and
// 951–963, trolleybuses 51–59, metro A/B/C, trains S1–S99 — so every key
// is the number the stop flag shows.
const LBL = new Map();

// night: trams 9x, buses 9xx and the X9x replacement night lines (user rule)
const NIGHT = /^(9\d{1,2}|X9\d)$/;
...
const S_BUS = new Set(SCOPE.bus), S_RAIL = new Set(SCOPE.rail);

// metro colours from the feed; the Esko trains all print PID's navy
const PID_COLORS = {};
for (const r of await readCsv(join(ROOT, 'data/gtfs/routes.txt'))) {
  const k = (r.route_short_name || '').trim();
  if (!k) continue;
  if (r.route_type === '1' && /^[0-9A-F]{6}$/i.test(r.route_color || '')) PID_COLORS[k] = '#' + r.route_color.toUpperCase();
  if (r.route_type === '2' && S_RAIL.has(r.route_id)) PID_COLORS[k] = '#251E62';
}
const isRailTrunk = (l) => l in PID_COLORS;

// --tram feeds three rail cfgs: A/B/C the metro, S… the trains, numbers the trams
const metroSel = tramLines.filter((l) => /^[ABC]$/.test(l));
const railSel = tramLines.filter((l) => /^S\d/.test(l));
const tramSel = tramLines.filter((l) => l !== 'all' && !/^[ABC]$/.test(l) && !/^S\d/.test(l));

const pidName = (n) => n.replace(/\s+/g, ' ').trim();

const MODES = [{
  mode: 'bus', label: 'buses', graphMode: 'road',
  // Prague and its ring — 5 × 5 tiles cut out of the Geofabrik extract (see
  ...
  feeds: [
    { tag: 'pid', dir: 'data/gtfs', routeTypes: ['3', '11'],
      skipRoute: (r) => !S_BUS.has(r.route_id), mapKey: (sn) => sn || null, nameFix: pidName },
  ],
}];
const tramAll = tramLines.length === 1 && tramLines[0] === 'all';
if (tramAll || tramSel.length) MODES.push({
  mode: 'tram', label: 'trams', osmFile: 'data/osm/praha-rail.json',
  graphMode: 'tram', railKeep: new Set(['tram']),
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : tramSel,
  feeds: [
    { tag: 'pid', dir: 'data/gtfs', routeTypes: ['0'], mapKey: (sn) => sn || null, nameFix: pidName, noFeedColors: true },
  ],
});
if (tramAll || metroSel.length) MODES.push({
  mode: 'tram', label: 'metro', osmFile: 'data/osm/praha-rail.json',
  graphMode: 'tram', railKeep: new Set(['subway']),
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : metroSel,
  feeds: [
    { tag: 'pid', dir: 'data/gtfs', routeTypes: ['1'], mapKey: (sn) => sn || null,
      lineColor: (k) => PID_COLORS[k], nameFix: pidName },
  ],
});
if (tramAll || railSel.length) MODES.push({
  // the Esko S-lines on railway=rail; allMetro gives every line the trunk
  // treatment. No allVariants: each S-line is one corridor.
  mode: 'tram', label: 'Esko trains', osmFile: 'data/osm/praha-rail.json',
  graphMode: 'tram', railKeep: new Set(['rail']),
  allMetro: true,
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : railSel,
  feeds: [
    { tag: 'pid', dir: 'data/gtfs', routeTypes: ['2'],
      skipRoute: (r) => !S_RAIL.has(r.route_id), mapKey: (sn) => sn || null,
      lineColor: (k) => PID_COLORS[k], nameFix: pidName },
  ],
});
